...
class MapProblem(GraphProblem):
    ...
    def set_additional_history_based_data(self):
        ...
        self.time_to_goal_history_based_data = pd.DataFrame.to_numpy(df.mean(axis=1))  # calculate mean & assign data
        ...
        for link in junction.outgoing_links:
            if link is not None:
                ...
                if self.cost_func_name == "distance":
                    ...
                    cost = link.compute_scheduled_time()
                elif self.cost_func_name == "current_time":
                    cost = link.compute_current_time()
